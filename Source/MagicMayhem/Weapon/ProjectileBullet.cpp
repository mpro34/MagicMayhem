...
{
	AMagiCharacter* OwnerCharacter = Cast<AMagiCharacter>(GetOwner());
	if (OwnerCharacter)
	{
		AMagiPlayerController* OwnerController = Cast<AMagiPlayerController>(OwnerCharacter->Controller);
		if (OwnerController)
		{
			...
			AMagiCharacter* HitCharacter = Cast<AMagiCharacter>(OtherActor);
	...
	// Call super, which destroys the projectile, call last.
	Super::OnHit(HitComp, OtherActor, OtherComp, NormalImpulse, Hit);
}
